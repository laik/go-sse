...
func TestClientSubscribe(t *testing.T) {
	...
	var cErr error
	go func() {
		cErr = c.Subscribe("test", func(msg *Event) error {
			if msg.Data != nil {
				events <- msg
			}
			return nil
...
func TestClientOnDisconnect(t *testing.T) {
	...
	go c.Subscribe("test", func(msg *Event) error { return nil })
...
func TestClientUnsubscribe401(t *testing.T) {
	...
	err := c.SubscribeRaw(func(ev *Event) error {
		// this shouldn't run
		assert.False(t, true)
		return nil
	})
...
func TestClientLargeData(t *testing.T) {
	...
	go func() {
		c.Subscribe("test", func(ev *Event) error {
			ec <- ev
			return nil
		})
	}()
